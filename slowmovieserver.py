...
def generate_frame(in_filename, out_filename, time, formattime):
    try:
        print(in_filename)
        print('starting ffmpeg')
        print( 'Resizing to: ' , width , ' X ' , height , ' at ' , formattime )
        (
            ffmpeg
            .input(in_filename, ss=time)
            .fullscreen_filter()
            .filter("scale", -2, height, force_original_aspect_ratio=1)
...
argsEpd.add_argument("-b", "--brightness", default=1.0, type=float, help="adjust image contrast (default: %(default)s)")
...
while True:
    ...
    formattime = ms_to_hours(int(currentFrame * videoInfo['frame_time']))

    # Use ffmpeg to extract a frame from the movie, letterbox/pillarbox it, and put it in memory as frame.bmp
    generate_frame(currentVideo, "frame.bmp", msTimecode, formattime)
